"""
Here a question generation model is trained with question complexity as an additional input.
Therefore, the model hopefully learns to generate question complexity controlled question generation.
A keyword is prepended to each input indicating 'easy', 'medium' or 'hard'.
"""
...
def main(args):
    ...
    complexities = ['easy']*num_middle + ['medium']*num_high + ['hard']*num_college
    ...
    for item in train_data:
        ...
        for qu_num in range(len(questions)):
            ...
            opts = options[qu_num]
            complexity = complexities[count]
            complexity_context = complexity + " [SEP] " + context
            passage_encodings_dict = tokenizer(complexity_context, truncation=True, max_length=MAXLEN_passage, padding="max_length")
