...
parser.add_argument('--complexity', type=str, default='easy', help='Load complexity level')
...
def main(args):
    ...
    for item in test_data[start:end]:
        ...
        complexity_context = args.complexity + " [SEP] " + context
        passage_encodings_dict = tokenizer(complexity_context, truncation=True, max_length=MAXLEN_passage, padding="max_length", return_tensors="pt")
    ...
    with open(args.save_path+str(args.part_num)+"_"+args.complexity+"_gen_questions.txt", 'w') as f:
        f.writelines("%s\n" % qu for qu in all_generated_questions)

    with open(args.save_path+str(args.part_num)+"_"+args.complexity+"_contexts.txt", 'w') as f:
