...
from models import ElectraMulti
...
def main(args):
    ...
    for inp_id, tok_typ_id, att_msk in dl:
        ...
        with torch.no_grad():
            curr_logits_qa, curr_logits_complexity = model(input_ids=inp_id, attention_mask=att_msk, token_type_ids=tok_typ_id)
        logits_qa += curr_logits_qa.detach().cpu().numpy().tolist()
        logits_complexity += curr_logits_complexity.detach().cpu().numpy().tolist()
    logits_qa = np.asarray(logits_qa)
    logits_complexity = np.asarray(logits_complexity)
    np.save(args.predictions_save_path + "logits_qa.npy", logits_qa)
    np.save(args.predictions_save_path + "logits_complexity.npy", logits_complexity)
